...
export default class DragSortableView extends Component{
    ...
    componentWillMount() {
        this._panResponder = PanResponder.create({
            onStartShouldSetPanResponder: (evt, gestureState) => {
                if(this.weShouldBeResponder) this.moveTouch(evt, gestureState);
                return false;
            },
            ...
            onMoveShouldSetPanResponder: (evt, gestureState) => {
                console.log('dragdrop onMoveShouldSetPanResponder', this.panResponderGranted, gestureState.dx);
                if(this.weShouldBeResponder) this.moveTouch(evt, gestureState);
                return this.weShouldBeResponder;
            },
            onMoveShouldSetPanResponderCapture: (evt, gestureState) => {
                console.log('dragdrop onMoveShouldSetPanResponderCapture');
                return this.weShouldBeResponder;
            },

            onPanResponderGrant: (evt, gestureState) => {
                console.log('dragdrop onPanResponderGrant');
                this.grantPageY = evt.nativeEvent.pageY;
                this.panResponderGranted = true
            },
            onPanResponderMove: (evt, gestureState) => {
                this.moveTouch(evt,gestureState);
            },
            onPanResponderRelease: (evt, gestureState) => {
                console.log('dragdrop onPanResponderRelease');
                this.panResponderGranted = false;
                this.endTouch(evt);
            },
            onPanResponderReject: (evt, gestureState) => {
                console.log('dragdrop onPanResponderReject');
                this.panResponderGranted = false;
                this.endTouch(evt);
            },
            onPanResponderTerminate: (evt, gestureState) => {
                ...
                // should be cancelled
                console.log('dragdrop onPanResponderTerminate');
                this.waitingForDragStartResolve = false;
                this.panResponderGranted = false;
                this.endTouch(evt);
            },

            onPanResponderTerminationRequest: (evt, gestureState) => {
                console.log('dragdrop onPanResponderTerminationRequest');
                this.waitingForDragStartResolve = false;
                return !this.weShouldBeResponder;
            },
    ...
    onPressIn(touchIndex) {
        console.log('dragdrop onPressIn');

        this.waitingForDragStartResolve = true;
        this.props.onDragStart().then(() => {
            console.log('dragdrop onDragStartResolved', this.waitingForDragStartResolve);

            if(!this.waitingForDragStartResolve) return;
            this.waitingForDragStartResolve = false;
            this.weShouldBeResponder = true;

            //防止拖动
            const fixedItems = this.props.fixedItems;
            if (fixedItems && fixedItems.length > 0 && fixedItems.includes(touchIndex)){
                return;
            }
    ...
    moveTouch (evt, gestureState) {
        ...
        if (this.weShouldBeResponder) {
            if(isNaN(this.startDY)) {
                this.startDY = isNaN(this.lastSeenDY) ? 0 : this.lastSeenDY;
                console.log('dragdrop startDY', this.startDY, isNaN(this.startDY));
            }
        } else {
            this.startDY = null;
        }
        this.lastSeenDY = gestureState.dy;
        if(isNaN(this.startDY)) return;
        ...
        if (this.touchCurItem) {
            ...
            if (this.startDY) {
                if (this.startDY > 0) {
                    const diff = this.startDY;
                    dy = dy - diff;
                } else {
                    const diff = Math.abs(this.startDY);
                    dy = dy + diff;
                }
            }
            ...
            if (this.touchCurItem.moveToIndex != moveToIndex ) {
                ...
                if (triggerHaptic && this.props.triggerHaptic) this.props.triggerHaptic();
    ...
    endTouch (nativeEvent) {
        this.weShouldBeResponder = false;
        this.startDY = undefined;
        this.lastSeenDY = undefined;
        console.log('dragdrop endTouch');
    ...
    onPressOut () {
        console.log('dragdrop onPressOut');
        if (this.props.onPressOut) this.props.onPressOut();
        if (!this.panResponderGranted) {
            this.endTouch()
        }
    }
    ...
    _renderItemView = () => {
        return this.state.dataSource.map((item,index)=>{
            ...
            const isDisabled = this.props.disabledItems.includes(key);
            return (
                <Animated.View
                    key={key}
